...
import Dashboard from './sprint_mps_dashboard';
...
const DEFAULT_CHECKS = Object.fromEntries(
  Object.entries(EVIDENCIAS).flatMap(([p, items]) =>
    items.map((_, i) => [`${p}_${i}`, false])
  )
);
...
function gerarBurndown(tarefas, capacidade) {
  ...
  return Array.from({ length: dias }, (_, i) => ({
    ...
    real:
      i < 7
        ? Math.max(0, Math.round(total - ((total * 0.55) / (dias - 1)) * i))
        : null,
  }));
}
...
export default function App() {
    ...
    async function load() {
      try {
        ...
        const results = await Promise.allSettled(
          keys.map((k) => window.storage.get(k))
        );
        const [t, imp, hist, ch, sp] = results.map((r) =>
          r.status === 'fulfilled' && r.value ? JSON.parse(r.value.value) : null
        );
        if (t) setTarefas(t);
  ...
  const updateTarefas = (v) => {
    setTarefas(v);
    save('sprint_tarefas', v);
  };
  const updateImpedimentos = (v) => {
    setImpedimentos(v);
    save('sprint_impedimentos', v);
  };
  const updateChecks = (v) => {
    setChecks(v);
    save('sprint_checks', v);
  };
  const updateSprint = (v) => {
    setSprint(v);
    save('sprint_info', v);
  };
  ...
  const handleCSV = async (e) => {
    ...
    if (!parsed.length) {
      setCsvMsg('Nenhuma tarefa encontrada. Verifique o formato do CSV.');
      return;
    }
  ...
  const finalizarSprint = () => {
    ...
    const entry = {
      ...
      pontos: done.reduce((s, t) => s + t.pontos, 0),
      velocidade: done.reduce((s, t) => s + t.pontos, 0),
    };
  ...
  const toggleImpedimento = (id) => {
    const updated = impedimentos.map((i) =>
      i.id === id
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Importe seu CSV do Jira para popular o board automaticamente
        </span>
        <button
        ...
        {csvMsg && (
          <span
            style={{
              fontSize: 12,
              color: csvMsg.includes('sucesso') ? '#1D9E75' : '#D85A30',
            }}
          >
            {csvMsg}
          </span>
      ...
      {tab === 'Sprint' && (
        <div>
          ...
          >
            {[
              { l: 'Total', v: tarefas.length },
              { l: 'Concluídas', v: done.length },
              { l: 'Em progresso', v: inProg.length },
              { l: 'A fazer', v: todo.length },
              { l: 'Pontos entregues', v: `${pontosDone}/${totalPontos}` },
            ].map((m) => (
              <div
                key={m.l}
                style={{
          ...
          {tarefas.length > 0 && (
            ...
            >
              ...
              {PROCESSOS_MPS.map((proc) => {
                const pts = tarefas
                  .filter((t) => t.processo === proc)
                  .reduce((s, t) => s + t.pontos, 0);
                const ptsD = tarefas
                  .filter((t) => t.processo === proc && t.status === 'done')
                  .reduce((s, t) => s + t.pontos, 0);
                if (!pts) return null;
                return (
                  <div
      ...
      {tab === 'Kanban' && (
        <div>
          {tarefas.length === 0 && (
            ...
            >
              Nenhuma tarefa ainda. Importe um CSV do Jira ou adicione
              manualmente.
            </div>
          )}
          <div
              ...
              >
                ...
                >
                  ...
                  <span
                    style={{
                      fontSize: 11,
                      background: STATUS_COLOR[st].bg,
                      color: STATUS_COLOR[st].text,
                      border: `0.5px solid ${STATUS_COLOR[st].border}`,
                      borderRadius: 12,
                      padding: '2px 8px',
                    }}
                  >
                    {tarefas.filter((t) => t.status === st).length}
                  </span>
                </div>
                {tarefas
            ...
            {!showFormT ? (
              ...
              >
                <input
                  value={novaT.titulo}
                  onChange={(e) =>
                    setNovaT((p) => ({ ...p, titulo: e.target.value }))
                  }
                  placeholder="Título da tarefa"
                  style={{ flex: 2, minWidth: 180, fontSize: 13 }}
                />
                <select
                  value={novaT.processo}
                  onChange={(e) =>
                    setNovaT((p) => ({ ...p, processo: e.target.value }))
                  }
                  style={{ fontSize: 13 }}
                >
                  {PROCESSOS_MPS.map((p) => (
                ...
                <input
                  ...
                  value={novaT.pontos}
                  onChange={(e) =>
                    setNovaT((p) => ({ ...p, pontos: Number(e.target.value) }))
                  }
      ...
      {tab === 'Evidências MPS.br' && (
        <div>
          ...
          >
            ...
            >
              {checkedCount} de {totalChecks} evidências confirmadas
            </p>
          ...
          {Object.entries(EVIDENCIAS).map(([proc, items]) => (
            ...
            >
              ...
              >
                ...
                >
                  {items.filter((_, i) => checks[`${proc}_${i}`]).length}/
                  {items.length}
                </span>
              </div>
              {items.map((item, i) => (
                ...
                >
                  <input
                    type="checkbox"
                    checked={!!checks[`${proc}_${i}`]}
                    onChange={() => {
                      const v = {
                        ...checks,
                        [`${proc}_${i}`]: !checks[`${proc}_${i}`],
                      };
                      updateChecks(v);
                    }}
                  />
                  <span
                    style={{
                      fontSize: 13,
                      color: checks[`${proc}_${i}`]
                        ? 'var(--color-text-secondary)'
                        : 'var(--color-text-primary)',
                      textDecoration: checks[`${proc}_${i}`]
                        ? 'line-through'
                        : 'none',
                    }}
                  >
        ...
        <div>
          ...
          {impedimentos.map((imp) => (
            <div
              ...
              style={{
                ...
                borderLeft: `3px solid ${
                  imp.status === 'aberto' ? '#BA7517' : '#1D9E75'
                }`,
                borderRadius: 12,
                padding: '1rem 1.25rem',
                marginBottom: 10,
            ...
            >
              ...
              >
                <p style={{ margin: '0 0 6px', fontSize: 14, flex: 1 }}>
                  {imp.descricao}
                </p>
                <span
                  style={{
                    fontSize: 11,
                    background: imp.status === 'aberto' ? '#FAEEDA' : '#EAF3DE',
                    color: imp.status === 'aberto' ? '#633806' : '#27500A',
                    borderRadius: 12,
                    padding: '2px 10px',
                    whiteSpace: 'nowrap',
                  }}
                >
                  {imp.status === 'aberto' ? 'Aberto' : 'Resolvido'}
                </span>
              </div>
              <div
                style={{
              ...
              <button
                onClick={() => toggleImpedimento(imp.id)}
                style={{
                  marginTop: 8,
                  fontSize: 12,
                  padding: '4px 12px',
          ...
          <div style={{ marginTop: 8 }}>
            {!showFormI ? (
              ...
              >
                <input
                  value={novaI.descricao}
                  onChange={(e) =>
                    setNovaI((p) => ({ ...p, descricao: e.target.value }))
                  }
                  placeholder="Descrição do impedimento"
                  style={{ flex: 2, minWidth: 200, fontSize: 13 }}
                />
                <input
                  value={novaI.responsavel}
                  onChange={(e) =>
                    setNovaI((p) => ({ ...p, responsavel: e.target.value }))
                  }
                ...
                <input
                  ...
                  value={novaI.prazo}
                  onChange={(e) =>
                    setNovaI((p) => ({ ...p, prazo: e.target.value }))
                  }
                  style={{ fontSize: 13 }}
                />
                <button
      ...
      {tab === 'Histórico' && (
        <div>
          {historico.length === 0 && (
            ...
            >
              Nenhum sprint finalizado ainda. Use o botão "Finalizar sprint" na
              aba Sprint.
            </div>
          )}
          {historico.length > 0 && (
